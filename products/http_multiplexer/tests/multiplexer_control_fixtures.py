...
def fixture(suffix="a"):
    from control_plane_kit_servers_http_multiplexer.configuration import MultiplexerControlConfiguration, multiplexer_control_declaration
    def key(family):
    ...
    config = MultiplexerControlConfiguration(
        target=target, runtime_id=core.NodeControlGraphReference(roles.RUNTIME, f"runtime-{suffix}"),
        declaration=multiplexer_control_declaration(), surface_issuer=f"surface-{suffix}", health_issuer=f"health-{suffix}",
