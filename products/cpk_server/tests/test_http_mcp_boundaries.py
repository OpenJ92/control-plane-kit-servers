...
class CpkServerHttpMcpBoundaryTests(unittest.TestCase):
    ...
    def test_public_ingress_resource_read_has_http_mcp_name_parity(self) -> None:
        from control_plane_kit_servers_cpk_server import (
            CpkServerHttpProcessBoundary,
            CpkServerMcpProcessBoundary,
        )
        ...
        http_result = http.handle(
            method="GET",
            path="/workspaces/workspace-a/public-ingress-resources",
            headers={"Authorization": "Bearer valid-token"},
            body=b"",
        )
        mcp_result = mcp.handle(
            ...
            message={
                ...
                "method": "resources/read",
                "params": {
                    "name": "list_public_ingress_resources",
        ...
        self.assertEqual(mcp_result.status, 200)
        requests = services[ControlPlaneServiceRole.READS].requests
        self.assertEqual([request.route_id for request in requests], [
            "read.public-ingress-resources",
            "read.public-ingress-resources",
        ])
        self.assertEqual(requests[0].path_parameters, {"workspace_id": "workspace-a"})
        self.assertEqual(requests[1].payload, {"workspace_id": "workspace-a"})

    def test_release_plan_has_http_mcp_name_parity_without_provider_values(self) -> None:
        from control_plane_kit_servers_cpk_server import (
            CpkServerHttpProcessBoundary,
            CpkServerMcpProcessBoundary,
        )
        ...
        payload = {
            ...
            "expected_reservation_version": 3,
        ...
        http_result = http.handle(
            method="POST",
            path=(
                "/workspaces/workspace-a/public-ingress-reservations/"
                "reservation-1/release-plan"
            ),
            headers={"Authorization": "Bearer valid-token"},
        ...
        mcp_result = mcp.handle(
            ...
            message={
                ...
                "method": "tools/call",
                "params": {
                    "name": "plan_public_ingress_reservation_release",
                    "arguments": {
        ...
        self.assertEqual([request.route_id for request in requests[-2:]], [
            "command.public-ingress-reservation.release-plan",
            "command.public-ingress-reservation.release-plan",
        ])
        encoded = repr((http_result.body, mcp_result.body))
        self.assertNotIn("api_token", encoded)
        self.assertNotIn("Cloudflare", encoded)
