...
def load_coordinates(path: Path) -> Mapping[str, Any]:
    ...
    _commit(upstreams, "control_plane_kit_core_commit")
    _commit(upstreams, "control_plane_kit_operations_commit")
    _commit(upstreams, "control_plane_kit_interpreters_commit")
    _commit(upstreams, "control_plane_kit_server_sdk_commit")
...
def generate_updates(coordinates: Mapping[str, Any]) -> dict[Path, bytes]:
    ...
    core_commit = str(upstreams["control_plane_kit_core_commit"])
    operations_commit = str(upstreams["control_plane_kit_operations_commit"])
    ...
    sdk_commit = str(upstreams["control_plane_kit_server_sdk_commit"])

    updates[PYPROJECT] = _replace_dependency_pins(
        PYPROJECT.read_text(encoding="utf-8"),
        sdk_commit=sdk_commit,
        core_commit=core_commit, operations_commit=operations_commit,
    ...
    updates[HELLO_SERVER_DOCKERFILE] = _replace_dependency_pins(
        ...
        sdk_commit=sdk_commit, core_commit=core_commit, operations_commit=operations_commit,
...
def _replace_dependency_pins(
    ...
    *,
    ...
    text = re.sub(
        r"https://github\.com/OpenJ92/control-plane-kit-server-sdk/archive/[0-9a-f]{40}\.zip",
        "https://github.com/OpenJ92/control-plane-kit-server-sdk/archive/"
        f"{sdk_commit}.zip",
        text,
    )
    for package, commit in (("core", core_commit), ("operations", operations_commit)):
        suffix = f"#subdirectory=control-plane-kit-{package}"
        text = re.sub(
            r"https://github\.com/OpenJ92/control-plane-kit/archive/[0-9a-f]{40}\.zip"
            + re.escape(suffix) + r"(?![A-Za-z0-9_-])",
            f"https://github.com/OpenJ92/control-plane-kit/archive/{commit}.zip{suffix}",
            text,
        )
    text = re.sub(
