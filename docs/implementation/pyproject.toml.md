...
Core and Operations use independent exact source commits. Interpreters6b1c75a9 and SDK2c5b5882 select the same Coreb79 archive; Secrets8273b7de selects that Core and SDK. Coordinate generation matches each package subdirectory. SDK[fastapi] selects FastAPI0.141.1 and Starlette1.6.0; Uvicorn and build dependencies still use ranges, so exact CPK pins do not make the whole environment a lockfile. Adoption requires checking actual selected contracts, not importing newest upstream assumptions into documentation.
