...
Core and Operations select the same exact source commit, while Interpreters selects its own exact commit plus cloudflare/docker/gateway/public-dns extras. Servers #193 selects Core/Operations95452249, Interpreters77c9a7f and SDK2b10d5a with its verification extra. Coordinate generation owns synchronized URL replacements. Both Interpreters and SDK select that same exact Core archive. Third-party FastAPI/Uvicorn and build dependencies use ranges, so exact CPK pins do not make the whole environment a lockfile. Adoption requires checking actual selected contracts, not importing newest upstream assumptions into documentation.

Dockerfile.test installs this combination on Python3.12. The owning package metadata test verifies installed direct-URL provenance for all four distributions in an isolated Python process and imports SDK verification/health/stdlib surfaces. This establishes installed source compatibility without starting a listener or granting authority. Product-specific Dockerfile SDK installation remains with each wrapper issue; this root package dependency does not qualify an existing published image.
